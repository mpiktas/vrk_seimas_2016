...
lm3<- duod %>% separate("duetas",c("pirmas","antras"),sep = "-") %>% 
...
lmf1 <- lmf %>% filter(!(apygardos_nr %in% lm3$apygardos_no)) %>% select(-pralaimejo)
lm4 <- lmf1 %>% bind_rows(lm3 %>%
                              select(apygardos_nr=apygardos_no,laimejo,skirtumas=vm_skirtumas))
lm5 <- lm4 %>% group_by(laimejo) %>% summarize(mandatai=n()) %>% arrange(-mandatai)
